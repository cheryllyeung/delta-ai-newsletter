...
1. fact_check：找出文章中所有事實聲明（企業名、數字、成效、時間），
   逐一比對是否能對應到來源全文。檢查範圍是整個 JSON，不是只有內文：
   chosen_headline、headline_candidates、chosen_subhead、sections 的
   heading、stats、card_summary 全部都算。每個聲明標記：
   - "supported"：來源明確支持
   - "unsupported"：來源找不到依據（幻覺）
   - "distorted"：來源有提但被誇大或扭曲

   標題要用最嚴的標準查（實測最容易失真的就是標題）：
   - 最高級與絕對詞（最大、首次、唯一、天差地遠）來源沒有明說就判
     "distorted"
   - 歸因式說法（「大家都以為」「過去把…當同一種」「戳破…的假設」）
     來源沒有這個對立面就判 "unsupported"，即使文章其餘內容都有依據
   - 程度詞漂移也算：來源說「不同」（distinct）被寫成「天差地遠」、
     「研究不足」被寫成「盲點」，都是 "distorted"

   供應商自家內容（官方 blog、新聞稿）要多一層檢查：來源自己的行銷說法
   被直接轉述成客觀事實、或把來源的產品定位再往前推一步（例如來源說
   「縮小差距」被寫成「等同」「取代」），都算 "distorted"。數字的性質
   不可漂移：定價不可寫成成本、單一地區價格不可寫成通用價格、最高規格
   不可寫成常態表現。
...
2. style_check：是否違反風格規範。逐條檢查：破折號、禁用詞、空泛形容詞、
   ...
   的一句話、paragraphs 裡是否出現條列符號或 markdown 粗體語法、是否混入
   繁體中文與英文以外的字元（簡體字、俄文等其他語言的單字、亂碼，實測
   出過「分子 профиля」這種殘留）。列出所有違規處。
   另外檢查「事實密度」：來源裡明確支撐主軸的關鍵數字與名稱（樣本數、
   分界值、標靶或產品名）如果幾乎都沒出現在文章裡，整篇靠比喻和評論
   撐篇幅，也算一條違規，rule 寫「關鍵事實缺席，修辭取代內容」並在
   location 列出被漏掉的關鍵事實。
