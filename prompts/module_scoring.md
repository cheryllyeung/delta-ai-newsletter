...
1. module_scores：對上面列出的每一個模組，給 0-10 分（可給小數）並附一句
   ...
   - 領域模組量的是「對台達這個事業的工作有沒有用」，不是「主題屬不屬於
     ...
     （不影響台達的軟體產品策略）
   - 職能模組量的是「這個部門的專業職務」直接用得上：人資模組要的是招募、
     考核、訓練流程的具體工具、做法或法規，不是「AI 讓員工更有生產力」
     這種全員通用的內容。全員都適用的辦公 AI 內容不專屬於任何職能模組，
     對各職能模組最多 3 分。例：「把 AI 嵌入日常工具提升員工體驗」對人資
     模組是 0-3 分（那是每個部門都能讀的通用內容，不是人資的專業職務）
   - 供應商行銷素材要壓分：內容只有願景、口號跟客戶讚美（transformation、
     empower 這類語彙），拿不出具體做法、架構或可驗證數據的，對所有模組
     最多 3 分。這類素材讀完做不了任何決定
   - 不要用間接推論鏈抬分：「任何硬體都需要電源散熱，所以這則裝置新聞跟
     零組件事業相關」這種腦補的下游影響不算相關。分數要來自內容直接談到
     的東西（規格、供應鏈、技術需求、法規），你替它想出來的影響不算。例：
     「某新創押注語音戒指」對消費性產品模組是 0-3 分，內容談的是產品策略，
     不是元件
   - 不要省略任何模組
